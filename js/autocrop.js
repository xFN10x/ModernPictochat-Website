/**
 * Message from MPC Dev: _FN10_ (i didnt make this script)
 *
 * This is from https://github.com/lqez/autocrop.js,
 * but i made it not crop on the x coord.
 */
...
(function (global, factory) {
  ...
  function autoCrop(original, target, options) {
    ...
    image.onload = () => {
      ...
      // Prepare functions
      const isBgColor = getIsBgColorFunc(
        options.bgColor,
      ...
      let bgRect = {
        //make end x the width to not crop the x (_FN10_)
        sx: 0,
        sy: -1,
        ex: original.width,
        ey: -1,
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          ...
          workSet.forEach((w) => {
            if (!w.func(pixel)) {
              //if (w.r.sx === -1 || w.r.sx > x) { w.r.sx = x } (_FN10_)
              if (w.r.sy === -1) {
                w.r.sy = y;
              }
              //if (w.r.ex < x) { w.r.ex = x } (_FN10_)
              if (w.r.ey < y) {
                w.r.ey = y;
              }
              w.c += 1;
      ...
      if (workSet[0].c < workSet[1].c) {
        ...
        if (
          width * height - workSet[0].c >=
          workSet[1].c * options.invertTolerance
        ) {
          needInvert = true;
        }
      }
      ...
      if (options.margin.slice(-1) === "%") {
        ...
        margin = parseInt(
          (Math.max(width, height) * parseInt(options.margin)) / 100.0
        );
      } else {
